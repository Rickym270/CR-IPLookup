...
'''
  ...
  Purpose:  Provides a tool to search up what the IP is registered for. The ability to switch
            ...
            The information is pulled from the database
'''
...
db = MySQLdb.connect(host=host, user=username, passwd = password, db = "IPChecker", unix_socket = socket)
cur = db.cursor()
...
if ip_address:
    ...
    rebuild_query = "SELECT count(1) from Network_Obj"
...
print(template.render(result=result,ip_address=ip_address,region=region,service_dictionary=service_dictionary,service_identifiers=service_identifiers, colspan_list=colspan_list,re_res=re_res,result_length=result_length,debug_list=debug_list,res2_count=res2_count,res_count=res_count,new_ip_start=new_ip_start,new_ip_end=new_ip_end))
